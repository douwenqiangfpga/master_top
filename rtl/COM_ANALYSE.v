...
always@(posedge clk_100M or negedge rst_n)begin
    if(!rst_n)begin
        ...
        if(w_spi_rx_done)
            r_need_spi_rx <= 1'b1;
        else if(r_cmd_state == `CMD_RX_SPI_TYPE)
            r_need_spi_rx <= 1'b0;
        else
            r_need_spi_rx <= r_need_spi_rx;
    end
end
...
always@(posedge clk_100M or negedge rst_n)begin
    if(!rst_n)
        r_spi_busy <= 1'b0;
    else if(w_spi_rx_done)
        r_spi_busy <= 1'b0;
    else if(r_cmd_rece_done && (r_spi_rx_bit_wide != 16'd0))
        r_spi_busy <= 1'b1;
end
    ...
    if(!rst_n)begin
        ...
        case(r_cmd_state)
            `CMD_IDLE:begin //空闲状态
                ...
                else if(r_need_rec_cmd && !w_spi_cmd_busy && !r_need_spi_rx) begin //需要接收ARM数据
                    r_cmd_state      <= `CMD_RD_TYPE;
                    r_rec_cmd_length <= 4; //8个字节
                end
                else if(r_need_spi_rx)begin //需要接收spi数据
