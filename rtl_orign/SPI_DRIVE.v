...
localparam      SPI_RX_CAP_FIRST = 16'd25;
localparam      SPI_SCLK_TOGGLE_CNT = 2'd1;  // 160MHz / (2 * 2) = 40MHz
...
always @(posedge clk_160M or negedge rst_n)begin
    if(!rst_n)begin
        ...
        if(spi_sclk_neg)begin
            if(r_spi_rx_bit_cnt == r_spi_rx_bit_wide_d0 - 16'd1)begin
                r_spi_rx_bit_cnt  <= 'd0;
                r_spi_rx_cnt_done <= 'd1;
                r_spi_rx_done     <= 'd1;
            end
            else if(~r_spi_rx_cnt_done&&r_spi_tx_cnt_done)begin
                if(r_spi_rx_bit_cnt >= SPI_RX_CAP_FIRST)begin
...
always @(posedge clk_100M or negedge rst_n)begin
    if(!rst_n)begin
        ...
        r_spi_fifo_data_valid_d1 <= r_spi_fifo_data_valid;
    end
end

assign  w_spi_fifo_rd_pulse = r_spi_fifo_rd_en;
assign  w_spi_rx_done_pulse = r_spi_fifo_data_valid_d1;
assign  o_spi_rx_done       = w_spi_rx_done_pulse;
assign  o_spi_tx_done       = r_spi_tx_cnt_done;
assign  o_spi_state         = 4'd0;
